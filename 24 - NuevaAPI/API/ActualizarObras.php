<?php 
    ...
    if  (!empty($_POST['nombre_obra']) and !empty($_POST['id_categoria']) and !empty($_POST['fecha_presentacion']) and !empty($_POST['lugar_presentacion'])  and !empty($_POST['duracion']) and !empty($_POST['actor_principal']))  {
        ...
        try {
            $consulta = $base_de_datos->prepare("UPDATE obra SET nombre_obra=:obr, id_categoria=:cat, fecha_presentacion=:pre, lugar_presentacion=:lug, duracion=:dur, actor_principal=:pri WHERE nombre_obra = :obr ");
